...
def lecture_detail(request, slug=None):
...
def lecture_list(request):
    ...
    # queryset_list = Lecture.objects.active()  # .order_by("-timestamp")
    # if request.user.is_staff or request.user.is_superuser:
    queryset_list = Lecture.objects.all()
...
def lecture_update(request, slug=None):
    if not request.user.is_staff or not request.user.is_superuser:
        raise Http404
    instance = get_object_or_404(Lecture, slug=slug)
    form = LectureForm(request.lecture or None, request.FILES or None, instance=instance)
    if form.is_valid():
        instance = form.save(commit=False)
        instance.save()
...
def lecture_delete(request, slug=None):
    if not request.user.is_staff or not request.user.is_superuser:
        raise Http404
    instance = get_object_or_404(Lecture, slug=slug)
    instance.delete()
